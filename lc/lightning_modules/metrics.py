...
class Iou(Metric):
    def __init__(self, num_classes: int = 7, normalize: bool = False):
        ...
        # Metric normally calculated on batch.
        # If true, final metrics (tp, fn, etc) will reflect average values per image
    ...
    def update(self, prediction: torch.Tensor, label: torch.Tensor):
        ...
        conf_mat = torch.bincount(
            self.num_classes * label + prediction, minlength=self.num_classes ** 2
        )
